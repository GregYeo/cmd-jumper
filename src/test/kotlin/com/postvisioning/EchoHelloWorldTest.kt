...
class EchoHelloWorldTest(
    private val mockMvc: MockMvc,
    private val objectMapper: ObjectMapper,
): FunSpec({
    context("""Echo "Hello World" test """){
        ...
        test("Response has got Exit Code=0"){
            assertThat(resJsonBody["exitCode"].intValue()).isEqualTo(0)
        }

        test("""Response has got stdout=["Hello World"]"""){
            val stdoutArray = resJsonBody.withArrayProperty("stdout")
            val stdoutLines = stdoutArray.map { it.textValue() }
            assertThat(stdoutLines).isEqualTo(listOf("Hello World"))
        }

        test("""Response has got stderr=[]"""){
            val stderr = resJsonBody.withArrayProperty("stderr")
